...
if __name__ == "__main__":
    for percent in [5, 16]:
        ...
        model = PandemicModel(n=400, width=50, height=50, masked_percentage=0.25, asymptomatic_probability=0.2,
                              infection_length=15, infection_probability=0.12, time_till_symptoms=percent, quarantine_length=14,
        ...
        for i in tqdm(range(num_epochs)):
            ...
            model = PandemicModel(n=400, width=50, height=50, masked_percentage=0.25, asymptomatic_probability=0.2,
                                  infection_length=10, infection_probability=0.12, time_till_symptoms=percent,
        ...
        plt.plot(infected_series, label=f"Infected")
        plt.legend(loc="upper right")
        plt.savefig(f"images/infected_at_10epochs_{percent}.png")
        plt.show()
        plt.plot(dead_series, label=f"Dead")
        plt.legend(loc="upper right")
        plt.savefig(f"images/dead_at_10epochs_{percent}.png")
        plt.show()
